...
class VentaController
{
	...
	public function crearventa()
	{
		...
		$venta->SucursalID 		= input("txtSucursalOk");
		$venta->estado 			= "Emitida";
		$venta->LogUsuarioID 	= $_SESSION["id"];
		$venta->guardar();
		...
		$arrayVentaPorxProd 		= json_decode($_POST['txtArrayVentaxProd'], true);
		...
		if (is_array($arrayIdProd) || is_object($arrayIdProd)) {
			foreach ($arrayIdProd as $indice => $valor) {
				...
				$detalleventa->Venta_Por 		= $arrayVentaPorxProd[$indice];
				$detalleventa->total 			= $arraysubTotalxProd[$indice];
				$detalleventa->FechaRegistro 	= date("Y-m-d H:i:s");;
	...
	public function cantidadCaya()
	{
		$cantidad 	= 0;
		$venta 		= Venta::all();
		foreach ($venta as $row) {
			if ($row->SucursalID == 2 && $row->Activo == 1) {
				$cantidad += 1;
			}
		}
		echo $cantidad;
	}
	public function cantidadAlexander()
	{
		$cantidad 	= 0;
		$venta 		= Venta::all();
		foreach ($venta as $row) {
			if ($row->SucursalID == 1 && $row->Activo == 1) {
				$cantidad += 1;
			}
		}
		echo $cantidad;
